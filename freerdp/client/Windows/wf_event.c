...
// 				}
				...
				{
// 					RECT _rc_cursor;
// 					_rc_cursor.left = wfc->vcursor_x - 3;
// 					_rc_cursor.right = wfc->vcursor_x + 3;
// 					_rc_cursor.top = wfc->vcursor_y - 3;
// 					_rc_cursor.bottom = wfc->vcursor_y + 3;
					//FillRect(hdc, &_rc_cursor, GetStockObject(BLACK_BRUSH));
					//wfc->memdc_vcursor
					//BitBlt(hdc, wfc->vcursor_x-(VCURSOR_W/2), wfc->vcursor_y-(VCURSOR_H/2), VCURSOR_W, VCURSOR_W, wfc->memdc_vcursor, 0, 0, SRCCOPY);
					BLENDFUNCTION _blendfunction = { AC_SRC_OVER, 0, 255, AC_SRC_ALPHA };
					AlphaBlend(hdc, wfc->vcursor_x - (VCURSOR_W / 2), wfc->vcursor_y - (VCURSOR_H / 2), VCURSOR_W, VCURSOR_W, wfc->memdc_vcursor, 0, 0, VCURSOR_W, VCURSOR_W, _blendfunction);

					wfc->vcursor_x_old = wfc->vcursor_x;
					wfc->vcursor_y_old = wfc->vcursor_y;
				}
// }}
