...
// Apex {{
BOOL playRecord(freerdp* instance, const unsigned char* data, size_t size)
{
	BOOL bRet = TRUE;

	wStream* s = Stream_New(data, size);

	//rdp_recv_callback...

	//fastpath_recv_updates(instance->context->rdp->fastpath, s);
	//if (rdp_recv_pdu(instance->context->rdp, s) < 0)
	//	bRet = FALSE;
	rdp_recv_callback(NULL, s, instance->context->rdp);
...
BOOL resetRdp(freerdp* instance)
{
